...
class SubTaskMoonItem(QGraphicsItem):
    def __init__(self, data, accent_color, orbit_radius, start_angle, speed, status_callback=None, sibling_count=1, parent_radius=100):
        super().__init__()
        self.data = data
    ...
    def _generate_unique_break(self):
        ...
        offset_x = random.uniform(-3, 3); offset_y = random.uniform(-3, 3)
        ...
        cutter.lineTo(math.cos(math.radians(angle - 50)) * r_outer, math.sin(math.radians(angle - 50)) * r_outer)
        cutter.closeSubpath(); stroker = QPainterPathStroker(); stroker.setWidth(random.uniform(2, 8)); stroker.setJoinStyle(Qt.PenJoinStyle.MiterJoin)
        ...
        for _ in range(num_shards):
            ...
            for _ in range(random.randint(3, 5)): shard_poly.append(QPointF(sx + random.uniform(-4,4), sy + random.uniform(-4,4)))
            ...
            if not final_shard.isEmpty():
                move_dist = random.uniform(2, 10); move_angle = math.atan2(sy, sx); dx = math.cos(move_angle) * move_dist; dy = math.sin(move_angle) * move_dist; final_shard.translate(dx, dy); self.shards.append(final_shard)
    def _generate_cosmic_dust(self):
        dust_count = random.randint(5, 15)
        for _ in range(dust_count):
            dist = random.uniform(self.radius * 1.2, self.radius * 3.0); angle = random.uniform(0, 360)
            dx = math.cos(math.radians(angle)) * dist; dy = math.sin(math.radians(angle)) * dist
            size = random.uniform(0.5, 3.0); poly = QPolygonF()
    ...
    def boundingRect(self): return self.rect.adjusted(-50, -50, 50, 50)
    ...
    def paint(self, painter, option, widget):
        ...
        if self.is_hovered: 
            if not self.is_done: base_color = QColor("#252525")
        ...
        if self.is_done: rim_color.setAlpha(180)
        painter.setPen(Qt.PenStyle.NoPen); painter.setBrush(QBrush(rim_color))
        for crater in self.craters: painter.drawPath(crater['outer'])

        pit_color = base_color.darker(150)
        if self.is_done: pit_color.setAlpha(200)
        painter.setBrush(QBrush(pit_color))
        for crater in self.craters: painter.drawPath(crater['inner'])
