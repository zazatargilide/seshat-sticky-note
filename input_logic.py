...
class InputLogic:
    ...
    def setup_shortcuts(self):
        ...
        QShortcut(QKeySequence("Ctrl+Alt+N"), self.mw).activated.connect(
            self.mw.menu_logic.create_new_note
        )

        # Навигация (делегируем в TreeLogic)
        tl = self.mw.tree_logic
        QShortcut(QKeySequence("Ctrl+Up"), self.mw).activated.connect(
            lambda: tl.move_item_vertical(-1)
        )
        QShortcut(QKeySequence("Ctrl+Down"), self.mw).activated.connect(
            lambda: tl.move_item_vertical(1)
        )
    ...
    def perform_undo(self):
        if self.mw.data.undo():
            self.mw.refresh_ui()

    def perform_redo(self):
        if self.mw.data.redo():
            self.mw.refresh_ui()
